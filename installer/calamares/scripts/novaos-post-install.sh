...
# Fallback if branding theme missing
if [[ ! -d /usr/share/sddm/themes/novaos ]]; then
  sed -i 's/^Current=novaos$/Current=breeze/' /etc/sddm.conf.d/zz-novaos-installed.conf || true
fi
...
rm -f /etc/xdg/autostart/novaos-installer-autostart.desktop 2>/dev/null || true
rm -f /usr/sbin/novaos-autostart-installer.sh 2>/dev/null || true
rm -f /etc/sudoers.d/novaos-calamares-live 2>/dev/null || true
