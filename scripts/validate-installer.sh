...
required=(
  ...
  scripts/novaos-autostart-installer.sh
  desktop/novaos-installer.desktop
  desktop/novaos-installer-autostart.desktop
...
if grep -q 'novaos-installer-autostart.desktop' "${ROOT}/scripts/build-iso.sh"; then
  pass "build-iso syncs Live autostart"
else
  fail "build-iso.sh must install Live Calamares autostart"
fi

if grep -q 'mode=live' "${SRC}/scripts/novaos-autostart-installer.sh"; then
  pass "autostart gated to live mode"
else
  fail "autostart script must check live install-state"
fi
