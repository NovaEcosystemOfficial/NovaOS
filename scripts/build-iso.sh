...
if [[ -d "${INSTALLER_SRC}" ]]; then
  ...
  cp -f "${INSTALLER_SRC}/scripts/novaos-autostart-installer.sh" \
        "${ROOT_OVERLAY}/usr/sbin/novaos-autostart-installer.sh"
  chmod 755 \
    "${ROOT_OVERLAY}/usr/sbin/novaos-post-install.sh" \
    "${ROOT_OVERLAY}/usr/sbin/novaos-autostart-installer.sh"
  cp -f "${INSTALLER_SRC}/desktop/novaos-installer.desktop" \
        "${ROOT_OVERLAY}/usr/share/applications/novaos-installer.desktop"
  cp -f "${INSTALLER_SRC}/desktop/novaos-installer-autostart.desktop" \
        "${ROOT_OVERLAY}/etc/xdg/autostart/novaos-installer-autostart.desktop"
else
